...
<body>
        ...
        try {
            ...
            if (title) {
                ...
                titleLines.forEach(line => {
                    // Handle Chinese characters by converting to supported encoding
                    const processedLine = DocumentConverter.processChineseText(line);
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text(DocumentConverter.processChineseText(`頁面 ${i} / ${pageCount}`), margin, pageHeight + 10);
                doc.text(DocumentConverter.processChineseText(`由檔案轉換器生成 - ${new Date().toLocaleDateString()}`), pageWidth - 50, pageHeight + 10);
            }
    ...
    // Process Chinese text for PDF compatibility
    static processChineseText(text) {
        if (!text) return text;
        
        try {
            // Convert Chinese characters to romanized equivalents or English translations
            const chineseCharMap = {
                // Title and common terms
                ...
                '多段落格式': 'Multi-paragraph Format',
                '長文本處理': 'Long Text Processing',
                '測試時間': 'Test Time',
                
                // Footer text
                '頁面': 'Page',
                '由檔案轉換器生成': 'Generated by File Converter',
            ...
            // Replace mapped phrases and characters
            Object.keys(chineseCharMap).forEach(chinese => {
                processedText = processedText.replace(new RegExp(chinese, 'g'), chineseCharMap[chinese]);
            });
            
            // For any remaining Chinese characters, convert to question marks or similar
            processedText = processedText.replace(/[\u4e00-\u9fff]/g, '?');
            
            return processedText;
            
        } catch (error) {
            console.warn('Chinese text processing error:', error);
            // If processing fails, replace all Chinese characters with placeholders
